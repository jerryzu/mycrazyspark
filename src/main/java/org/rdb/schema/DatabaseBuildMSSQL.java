...
public class DatabaseBuildMSSQL {
			...
			for (String className : classNames) {
				...
				for (Field field : cl.getDeclaredFields()) {
					...
					if (annotations[0] instanceof SQLBoolean) {
						...
						if (columnName.length() <= 0) {
							columnName = field.getName() + " smallint";
						}
					...
					if (annotations[0] instanceof SQLDouble) {
						...
						if (columnName.length() <= 0) {
							columnName = field.getName() + " decimal";
						} else {
							columnName += " decimal";
						}
					...
					if (annotations[0] instanceof SQLTimestamp) {
						...
						if (columnName.length() <= 0) {
							columnName = field.getName() + " datetime2";
						}
