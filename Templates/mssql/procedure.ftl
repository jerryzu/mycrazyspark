...
BEGIN
	...
	DECLARE @v_StartMonth VARCHAR(12)
	DECLARE @v_EndMonth VARCHAR(12)
	...
	SELECT @v_StartMonth = CONVERT(varchar(6), @pd_startdate, 112)
	SELECT @v_EndMonth = CONVERT(varchar(6), @pd_enddate, 112)
	...
	INSERT INTO Cognos.TS_MAN_ETL_LOG_Ex 
	VALUES (@v_log_etl_program_type, @v_log_etl_program_name, @v_log_db_level, @v_log_source_table, @v_log_dest_table
		...
		, @v_log_DelRecord, @v_log_InsRecord)	
END
GO
</#list>