 ...
 */
  ...
  cursor cur_${p.procName?substring(4)} is select  m.ROWID 
    from ${p.dependFieldMappinglist[0].targetTableOwner!" "}.${p.dependFieldMappinglist[0].targetTableName!" "} m
   		inner join ${p.masterKeyMappinglist[0].sourceTableOwner!" "}.${p.masterKeyMappinglist[0].sourceTableName!" "}_ETL l
   		  on <#list p.masterKeyMappinglist as FieldProp>m.${FieldProp.targetFieldName!" "} = l.${FieldProp.sourceFieldName!" "} <#if FieldProp_has_next> and </#if><#rt>
		</#list>
    order by m.rowid;  
  </#list>
...
begin
  ...
  loop     
    fetch   cur_${p.procName?substring(4)} bulk collect into v_rowid  limit 5000;--每次处理5000行，也就是每5000行一提交
    exit when v_rowid.count=0;
    forall i in v_rowid.first..v_rowid.last
      delete from ${p.dependFieldMappinglist[0].targetTableOwner!" "}.${p.dependFieldMappinglist[0].targetTableName!" "}  
      --update ${p.dependFieldMappinglist[0].targetTableOwner!" "}.${p.dependFieldMappinglist[0].targetTableName!" "}  set D_DELETE_DATE = trunc(sysdate)
