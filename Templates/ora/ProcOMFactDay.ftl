 ...
 */
  ...
  VD_CURRENTQUARTER   DATE;
  VD_CURRENTYEAR      DATE;
  VD_CURRENTCALENDARID NUMBER;
  VD_CURRENTYEARMONTH NUMBER;
begin
  ...
  -- TO DO IT  
  VN_TMPLOGID := TOOL_UTIL.FN_ETL_LOG_DEBUG(-1,VS_PRCOWNER,VS_PRCNAME,VS_PRCPARAM,PS_BATCH,'exec',SYSDATE,NULL,0,'插入 数据到表${p.dependTargetTablelist[0].targetTableOwner!" "}.${p.dependTargetTablelist[0].targetTableName!" "}',1);
  
  VD_CURRENTDAY       := vd_startdate;
  WHILE VD_CURRENTDAY <= vd_enddate
  LOOP
  begin
    VD_CURRENTMONTH := TRUNC(VD_CURRENTDAY, 'MM');
    VD_CURRENTQUARTER :=  TRUNC(VD_CURRENTDAY, 'Q');
    VD_CURRENTYEAR :=  TRUNC(VD_CURRENTDAY, 'Y');
    VD_CURRENTCALENDARID := TO_CHAR(VD_CURRENTDAY, 'YYYYMMDD');
    VD_CURRENTYEARMONTH := TO_CHAR(VD_CURRENTDAY, 'YYYYMM');
    
    --CLEAR REDUNDANT DATA
    IF OM_UTIL.FN_IS_INITDATA = 0 THEN
      SELECT COUNT(1) INTO vn_tmpcount FROM ${p.dependTargetTablelist[0].targetTableOwner!" "}.${p.dependTargetTablelist[0].targetTableName!" "} t WHERE t.D_EVENTDATE BETWEEN VD_CURRENTDAY AND VD_CURRENTDAY;
      vn_tmplogid := TOOL_UTIL.FN_ETL_LOG_DEBUG(-1,vs_prcOwner,vs_prcName,vs_prcParam,ps_batch,'exec',sysdate,null,0,'TRUNCATE ${p.dependTargetTablelist[0].targetTableOwner!" "}.${p.dependTargetTablelist[0].targetTableName!" "}的分区'||TO_CHAR(VD_CURRENTDAY, 'YYYYMMDD'),null);
      COM_UTIL.PRC_SQL_TRUNCATE_MONTH('${p.dependTargetTablelist[0].targetTableOwner!" "}','${p.dependTargetTablelist[0].targetTableName!" "}',VD_CURRENTDAY,VD_CURRENTDAY,'YYYYMMDD','PART_');
      vn_tmplogid := TOOL_UTIL.FN_ETL_LOG_DEBUG(vn_tmplogid,vs_prcOwner,vs_prcName,vs_prcParam,ps_batch,'exec',null,sysdate,vn_tmpcount,'TRUNCATE成功!',1);
--      OTHER METHOD
...
--      DELETE ${p.dependTargetTablelist[0].targetTableOwner!" "}.${p.dependTargetTablelist[0].targetTableName!" "} t WHERE t.D_EVENTDATE BETWEEN VD_CURRENTDAY AND VD_CURRENTDAY;
...
--      vn_tmplogid := TOOL_UTIL.FN_ETL_LOG_DEBUG(vn_tmplogid,vs_prcOwner,vs_prcName,vs_prcParam,ps_batch,'exec',null,sysdate,vn_tmpcount,'清除操作成功!',1);
  ...
  -- TO DO IT      
  	VN_TMPLOGID := TOOL_UTIL.FN_ETL_LOG_DEBUG(-1,VS_PRCOWNER,VS_PRCNAME,VS_PRCPARAM,PS_BATCH,'exec',SYSDATE,NULL,0,'插入分区'|| TO_CHAR(VD_CURRENTDAY, 'YYYYMMDD') || '数据到表TDM_EVT_FACT_CTRL_DSHBRD_DAY的分区',NULL);
  ...
  WHERE<#list sourceTable.incCaptureFieldlist as incCaptureField> ${incCaptureField.fieldName!""} BETWEEN {VD_CURRENTDAY VD_CURRENTMONTH VD_CURRENTQUARTER VD_CURRENTYEAR} AND VD_CURRENTDAY</#list>
  </#list> 
