 ...
 * =================================相关表====================================
 * 依赖过程
 * 数据源  <#list p.dependencylist as depend><#if depend.dependencyRefType == "SOURCETABLE"> ${depend.dependencyName!" "} </#if> </#list>
 * 结果表  <#list p.dependencylist as depend><#if depend.dependencyRefType == "TARGETTABLE"> ${depend.dependencyName!" "} </#if> </#list>
 * =================================更新历史====================================
 *  创建时间	开发人员	动作	修改原因
...
begin
  ...
  -- TO DO IT  
	...
	MERGE INTO ${p.dependFieldMappinglist[0].targetTableOwner!" "}.${p.dependFieldMappinglist[0].targetTableName!" "} T 
    	USING (SELECT M.*, RANK() OVER(PARTITION BY ${p.dependSourceTablelist[0].bizKey!" "}
      		ORDER BY ${p.dependSourceTablelist[0].masterKey!" "} DESC) RANKNO
    	FROM ${p.dependFieldMappinglist[0].sourceTableOwner!" "}.${p.dependFieldMappinglist[0].sourceTableName!" "} M
    	) S
      		ON (T.${p.dependTargetTablelist[0].masterKey!" "} = S.${p.dependSourceTablelist[0].masterKey!" "} AND S.RANKNO = 1)   
	WHEN MATCHED THEN  
	  UPDATE  
       SET 
	  <#list p.dependFieldMappinglist as FieldProp>
	  <#if FieldProp.sourceFieldName??>  
	  <#if FieldProp.sourceFieldName != p.dependTargetTablelist[0].masterKey>
	  <#if FieldProp_index != 0>,</#if><#rt>
      t.${FieldProp.targetFieldName!" "} = <#lt><#if FieldProp.sourceExpression??>${FieldProp.sourceExpression!" "}<#else>${FieldProp.sourceFieldName!" "}</#if> <#t>		
	...
	WHEN NOT MATCHED THEN  
	  ...
	  <#list p.dependFieldMappinglist as FieldProp>
	  <#if FieldProp_index != 0>,</#if><#rt>
  	  <#if FieldProp.sourceExpression??>${FieldProp.sourceExpression!" "}<#else><#if FieldProp.sourceFieldName??>s.${FieldProp.sourceFieldName!" "}<#else>NULL</#if></#if> <#t>		
		--${FieldProp_index+1}	${FieldProp.sourceFieldDesc!" "}<#lt>
      </#list>)
    ;
