 ...
 */
  ...
  -- 开发人员定义变量
  vd_currentDay date;--当前日期
  vd_currentDayLast date;--当前日期 
  vd_currentMonth date;--当前日期
  vd_currentMonthLast date;--当前日期 
begin
  ...
  */
  ...
  if(vd_enddate > sysdate) then vd_enddate := trunc(sysdate); end if;

  vd_currentDay := vd_startdate;
  /**---------------循环到天调用*/
  WHILE vd_currentDay <= vd_enddate LOOP
  BEGIN
    vd_currentDayEnd := vd_currentDay + 86399 / 86400;
  -- TO DO IT
  -- INSERT INTO
  -- VN_TMPLOGID := TOOL_UTIL.FN_ETL_LOG_DEBUG(-1,VS_PRCOWNER,VS_PRCNAME,VS_PRCPARAM,PS_BATCH,'exec',SYSDATE,NULL,0,'插入 数据到表RTDB_DWH.TDM_EVT_PAYMENT_MS_PAYCHARGE1',1);
